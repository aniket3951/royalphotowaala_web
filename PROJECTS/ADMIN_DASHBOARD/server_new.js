  ...
  try {
    ...
    res.json({
      ...
      images: images.map(img => ({
        ...
        url: img.url ? img.url.replace('/static/uploads/', '/uploads/') : `/uploads/${img.public_id}`,
...
app.delete('/api/gallery-images/:filename', loginRequired, async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/gallery/${filename}`, {
      method: 'DELETE'
    });
    ...
    if (assets.logo) {
      res.json({
        ...
        logo: {
          ...
          url: assets.logo.url.replace('/static/uploads/', '/uploads/')
        }
      });
    } else {
    ...
    res.json({
      ...
      images: images.map(img => {
        ...
        return {
          id: img.id,
          filename: filename,
          url: img.url ? img.url.replace('/static/uploads/', '/uploads/') : `/uploads/${filename}`,
...
app.delete('/api/home-images/:filename', loginRequired, async (req, res) => {
  ...
  try {
    ...
    const getResponse = await fetch('http://localhost:5000/api/home-images');
    ...
    const response = await fetch(`http://localhost:5000/api/home-images/${imageToDelete.id}`, {
      method: 'DELETE'
    });
